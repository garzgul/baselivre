...
import baselivre.ui.gestionstock.*;
 ...
 * @author cdi116
 */
public class jiRefLivre extends javax.swing.JInternalFrame {
    
    private String sqlConnexion;
    private InterfaceRoot ir;
    private Vector table;
    private Vector titres;
    private Vector listeLivre;
     ...
     * @param ir
     * L'interface root permet de recuperer les parametres de connexion a l
     * la base SQL ainsi que de rattacher la frame au desktop de la frame root.
     */
    public jiRefLivre(InterfaceRoot ir) {
        this.ir = ir;
    ...
    private void initComponents() {
        ...
        setClosable(true);
        setTitle("Gestion des Referencements Livre");
        ...
        jBDeref.setText("Dereferencement");
        ...
        jBDeref.setBounds(430, 510, 130, 23);

        jBRef.setText("Referencement");
        ...
        jBRef.setBounds(120, 510, 130, 23);

        setBounds(0, 0, 765, 795);
    ...
    private void jBDerefActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBDerefActionPerformed
        ...
        if (jTable1.getSelectedRowCount()!= 1){
            ...
            ResultSet rs = null;
            try {
                rs = stmt.executeQuery("SELECT * from livre where livre_isbn13 = '" + ISBN13 + "'");
                int idLivre = rs.getInt("id_livre");
                stmt.executeUpdate("Update livre set livre_actif = 0 where id_livre = "+idLivre);
                rs.close();
                stmt.close();
    ...
    private void jBRefActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jBRefActionPerformed
        ...
        if (jTable1.getSelectedRowCount() != 1) {
            ...
            try {
                rs = stmt.executeQuery("SELECT * from livre where livre_isbn13 = '" + ISBN13 + "'");
                int idLivre = rs.getInt("id_livre");
                stmt.executeUpdate("Update livre set livre_actif = 1 where id_livre = " + idLivre);
                rs.close();
                stmt.close();
    ...
    // initialisation de la combobox soustheme
